...
import { Search, Save, Trash2, ArrowUpRight } from 'lucide-react';
...
export default function StockIssuePage() {
  ...
  const [recentIssues, setRecentIssues] = useState<InventoryTransaction[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setRecentIssues(list.filter(tx => tx.type === 'ISSUE').slice(0, 10));
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await inventoryTransactionService.createStockMovement({
        ...
        notes: reason.trim() || 'صرف مخزني',
        createdBy: 'system'
      });
      setItems([]);
      setReference('');
      setReason('');
      alert('تم تسجيل صرف البضاعة بنجاح.');
    } catch (error: any) {
      console.error(error);
      alert(error.message || 'حدث خطأ أثناء تسجيل الصرف');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-8 pb-20" dir="rtl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">صرف بضاعة</h2>
          <p className="text-gray-500 mt-1">سجل صرف المخزون للأنشطة الداخلية أو الاستهلاك.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-8">
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8 space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-widest text-gray-400">مرجع الصرف</label>
            <input value={reference} onChange={e => setReference(e.target.value)} placeholder="رقم الإذن أو سبب الصرف"
              className="w-full bg-gray-50 rounded-2xl px-4 py-4 text-sm font-bold outline-none border border-gray-200" />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-widest text-gray-400">سبب الصرف</label>
            <textarea value={reason} onChange={e => setReason(e.target.value)} rows={3} placeholder="مثلاً: استهلاك داخلي، نقل داخلي، هدر"
              className="w-full bg-gray-50 rounded-2xl px-4 py-4 text-sm font-bold outline-none border border-gray-200" />
          </div>
          <div className="space-y-2">
            <p className="text-sm font-black text-gray-600 uppercase tracking-widest">المنتجات المصروفة</p>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm text-gray-500">إجمالي الكمية المصروفة</p>
            ...
            <button onClick={handleSubmit} disabled={submitting || items.length === 0}
              ...
              {submitting ? 'جارٍ الحفظ...' : 'تأكيد الصرف'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-500 text-sm">اضغط على منتج لإضافته لقائمة الصرف.</p>
            </div>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-black text-gray-900">آخر 10 صرفيات</h3>
        </div>
        {recentIssues.length === 0 ? (
          <div className="text-center py-16 text-gray-400">لا توجد صرفيات سابقة.</div>
        ) : (
          <div className="space-y-3">
            {recentIssues.map(tx => (
              <div key={tx.id} className="p-4 rounded-3xl border border-gray-100 bg-gray-50">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="font-black text-gray-900">{tx.reference || 'صرف مخزني'}</p>
                    <p className="text-sm text-gray-500">{tx.notes || 'صرف بضاعة'}</p>
                  </div>
                  <span className="text-xs font-black text-gray-500">{tx.items.length} صنف</span>
                </div>
              </div>
            ))}
          </div>
